...
# compute_hydro_signatures_camels enables the computation of the signatures selected
# for the original CAMELS paper
# (Addor et al., 2017, HESS).

# For some signatures, several formulations have been implemented and the resulting estimates are #returned as a
# data.frame. Alternative formulations can be added. The objective is to assess the
...
source(paste('D:/working_directory)/time_tools.R',sep='')) # for month2sea and get_hydro_year
#source(paste(dir_r_scripts,'time_tools.R',sep='')) # for month2sea and get_hydro_year

# STEP 2 -  library to read matlab data formats into R
library(R.matlab)
library(lfstat)
library(readtext)
library(hdf5r)
### WRAPPER AND PARAMETER VALUES TO COMPUTE STANDARD CAMELS HYDROLOGICAL
###SIGNATURES
...
# low_q_dur      - Average duration of low-flow events (number of consecutive days <0.2 times the
# mean daily flow)
# zero_q_freq    - Frequency of days with Q = 0 mm/day

### STEP 3 - FUNCTIONS FOR INDIVIDUAL SIGNATURES
...
#(between the log-transformed 33rd and 66th streamflow percentiles)
...
comp_i_bf<-function(q,d,alpha,passes,tol){
  ...
  if(sum(is.na(q))/length(q)>=tol){ # not using avail_data here, since it would remove time steps
# with NA, and thereby alter the consistency of the time series and bias the baseflow separation
...
## STEP 4 - compute hydro signatures camels
...
q <- readMat("discharge.mat")
q <- q[[1]]
p <- readMat("precipitation.mat")
p <- p[[1]]
d <- read.csv(file =’dates.csv')
head(d)
hy_cal <- 'sep_br'
tol <- 2000

# transform data into atomic vector
d<-array(as.data.frame(d))
q<-array(as.data.frame(q))
p<-array(as.data.frame(p))

## STEP 6 - Calculates the result of the indices using the previous functions

start_time <- as.numeric(as.numeric(Sys.time()), digits=2) # place at start
rows <- dim(q)[2]
index<- data.frame(
  q_mean = numeric(rows), runoff_ratio = numeric(rows), stream_elas = numeric(rows),
  slope_fdc=numeric(rows), baseflow_index_landson=numeric(rows), baseflow_index_lfstat=numeric(rows),
  hfd_mean=numeric(rows), Q5=numeric(rows), Q95=numeric(rows), high_q_freq=numeric(rows), high_q_dur=numeric(rows),
  low_q_freq=numeric(rows), low_q_dur=numeric(rows),zero_q_freq=numeric(rows)
  )
i<-1
for (i in 1:dim(q)[2]) {
  date<-as.Date(d[,i+1], format="%Y-%m-%d")  # transform date table into format D
  prec<-unlist(p[,i], use.names=FALSE)
  flow<-unlist(q[,i], use.names=FALSE)
  print(paste("Calculando minibacia ", i, "de ", dim(q)[2]))
  result<- compute_hydro_signatures_camels(flow,prec,date,tol,hy_cal)
  index[i,]<-result
  }

end_time <- as.numeric(as.numeric(Sys.time()), digits=2) # place at end
